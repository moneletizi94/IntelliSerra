...
object Simulation {
  ...
  final case class DoubleSinSample(stdValue: Double, delta: Double) extends Sample[DoubleType] {
  ...
  final case class StringSample(probabilityMap: (String, Double)*) extends Sample[StringType] {
    private val random = RandomWithProbability(probabilityMap.toMap)
    override def sample: StringType = random.nextItem
  }

  final case class RandomWithProbability[T](items: Map[T, Double]) {
